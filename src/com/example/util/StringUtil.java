...
public final class StringUtil {
  ...
  */
  ...
  public static String cutoffString(String metadata){
	  String substitute = "...";
	  int length = 9;
	  if(metadata.length() > length){
		  return metadata.subSequence(0, length - 1) + substitute;
	  }else{
		  return metadata;
	  }
  }
  
  public static String expansionString(String metadata){
	  int length = 2;
	  StringBuilder sb = new StringBuilder();
	  if(metadata.length() ==  length){
		  sb.append(metadata.substring(0, 1));
		  sb.append("  ");
		  sb.append(metadata.substring(1, 2));
		  return sb.toString();
	  }else{
		  return metadata;
	  }
