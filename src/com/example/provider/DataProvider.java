...
import com.example.provider.DataProviderContract.MessageEntry;
import com.example.provider.DataProviderContract.TargetEntry;
...
public class DataProvider extends ContentProvider {
    ...
    static {
        ...
        // Adds a URI "match" entry that maps target content URIs to a numeric code
        sUriMatcher.addURI(
                DataProviderContract.AUTHORITY,
                TargetEntry.TARGET_TABLE_NAME,
                TARGET_QUERY);

        // Adds a URI "match" entry that maps modification date content URIs to a numeric code
        sUriMatcher.addURI(
            DataProviderContract.AUTHORITY,
            MessageEntry.MESSAGE_TABLE_NAME,
            MESSAGE_QUERY);
        
        // Specifies a custom MIME type for the target table
        sMimeTypes.put(
                TARGET_QUERY,
    ...
    private class DataProviderHelper extends SQLiteOpenHelper {
        ...
        private void dropTables(SQLiteDatabase db) {
            ...
            db.execSQL("DROP TABLE IF EXISTS " + TargetEntry.TARGET_TABLE_NAME);
            db.execSQL("DROP TABLE IF EXISTS " + MessageEntry.MESSAGE_TABLE_NAME);
        }
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL(TargetEntry.CREATE_TARGET_TABLE_SQL);
            db.execSQL(MessageEntry.CREATE_MESSAGE_TABLE_SQL);
    ...
    public int bulkInsert(Uri uri, ContentValues[] insertValuesArray) {
    	...
    	Log.e("master", sUriMatcher.toString());
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case TARGET_QUERY:
                ...
                localSQLiteDatabase.delete(TargetEntry.TARGET_TABLE_NAME, null, null);
                ...
                for (int i = 0; i < numImages; i++) {
                    ...
                    localSQLiteDatabase.insert(TargetEntry.TARGET_TABLE_NAME,
                    		TargetEntry.COLUMN_NAME, insertValuesArray[i]);
                }
